...
const BATTERY_THRESHOLDS = [
  { level: 20, urgency: Notifd.Urgency.NORMAL, title: "Battery Low", body: "Battery at 20%" },
  { level: 10, urgency: Notifd.Urgency.NORMAL, title: "Battery Very Low", body: "Battery at 10%" },
  { level: 5, urgency: Notifd.Urgency.CRITICAL, title: "Battery Critically Low", body: "Battery at 5%" },
]

let lastNotifiedLevel: number | null = null
...
function initBatteryWarnings() {
  ...
  const checkBattery = () => {
    ...
    for (const threshold of BATTERY_THRESHOLDS) {
      if (pct <= threshold.level) {
        if (lastNotifiedLevel !== threshold.level) {
          lastNotifiedLevel = threshold.level
          sendBatteryNotification(threshold.title, threshold.body, threshold.urgency)
        }
        break
      }
    }

    if (pct > BATTERY_THRESHOLDS[0].level) {
      lastNotifiedLevel = null
    }
  }
  ...
  battery.connect("notify::charging", checkBattery)
}

function sendBatteryNotification(title: string, body: string, urgency: Notifd.Urgency) {
  try {
    execAsync(["notify-send", "-u", urgency === Notifd.Urgency.CRITICAL ? "critical" : "normal", "-i", "battery-caution-symbolic", title, body]).catch((e) => {
      console.error("Failed to send battery notification:", e)
    })
  } catch (e) {
    console.error("Failed to send battery notification:", e)
  }
...
export function BatteryPopup() {
  ...
  const refreshProfile = () => {
    const _current = powerProfiles.activeProfile
  }
  ...
  if (shouldBlockPerformance(battery) && powerProfiles.activeProfile === "performance") {
    powerProfiles.activeProfile = "balanced"
  }
